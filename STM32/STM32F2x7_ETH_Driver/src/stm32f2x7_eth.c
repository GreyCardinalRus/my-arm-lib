...
uint16_t ST802RT1_PHY_Address; //STEVAL-PCC010V2 
...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  //Read out the PHY ID with the default MDIO address - PHYAddress: STEVAL-PCC010V2 uses ST802RT1
  if (!(ETH_ReadPHYRegister(PHYAddress, PHY_PHYID1) == ST_OUI_H))
  {
  PHYAddress = 0; //STEVAL-PCC010V2 
  do { //Try to detect the PHY under different MDIO address
    if (ETH_ReadPHYRegister(PHYAddress, PHY_PHYID1) == ST_OUI_H) break; //STEVAL-PCC010V2 
      PHYAddress++; //STEVAL-PCC010V2 
  } while(PHYAddress < 0x20); //STEVAL-PCC010V2 

  if (PHYAddress > 0x1F)  return ETH_ERROR; //STEVAL-PCC010V2 

  }
   
  ST802RT1_PHY_Address = PHYAddress; // VJ ST802RT1_PHY_Address used in fs.c for the html pages
   
  /* Put the ST802RT1 PHY in reset mode */
  if(!(ETH_WritePHYRegister(ST802RT1_PHY_Address, PHY_BCR, PHY_Reset)))//STEVAL-PCC010V2  
  {
    /* Return ERROR in case of write timeout */
    return ETH_ERROR; //STEVAL-PCC010V2 
  }


    /* //STEVAL-PCC010V2 Poll the PHY for reset in progress */
      do                                                                                          //STEVAL-PCC010V2 
    {                                                                                             //STEVAL-PCC010V2 
      timeout++;                                                                                  //STEVAL-PCC010V2 
    } while ((ETH_ReadPHYRegister(PHYAddress, PHY_BCR) & PHY_Reset) && (timeout < PHY_READ_TO));  //STEVAL-PCC010V2 
    /* Return ERROR in case of timeout */                                                         //STEVAL-PCC010V2 
    if(timeout == PHY_READ_TO)                                                                    //STEVAL-PCC010V2 
    {                                                                                             //STEVAL-PCC010V2 
      return ETH_ERROR;                                                                           //STEVAL-PCC010V2 
    }                                                                                             //STEVAL-PCC010V2 
    /* Reset Timeout counter */                                                                   //STEVAL-PCC010V2 
    timeout = 0;                                                                                  //STEVAL-PCC010V2 

  
//  /* Delay to assure PHY reset */
//  _eth_delay_(PHY_RESET_DELAY); //STEVAL-PCC010V2 
  
    
   /*Enable Auto MDIX for ST802RT1*/ //STEVAL-PCC010V2 
  RegValue = ETH_ReadPHYRegister(PHYAddress, PHY_TSTAT);  //STEVAL-PCC010V2 
  ETH_WritePHYRegister(PHYAddress, PHY_TSTAT, (RegValue & PHY_MDIX_Disable_mask)); //STEVAL-PCC010V2 
